...
export default function Dashboard() {
  ...
  const [applyingGames, setApplyingGames] = useState(new Set()); // IDs de partidos en proceso
  ...
  async function loadCanchas() {
    try {
      ...
      if (res.ok) {
        ...
        logger.log("Error al cargar canchas, status:", res.status);
        // Intentar fallback sin /api/
        try {
          logger.log("Intentando fallback: /canchas");
          ...
          if (fallbackRes.ok) {
            ...
            logger.log("Canchas cargadas desde fallback:", fallbackData);
            setCanchas(fallbackData);
          } else {
            logger.log("Fallback también falló, status:", fallbackRes.status);
          ...
          logger.error("Error en fallback de canchas:", fallbackError);
  ...
  async function loadUbicaciones() {
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        logger.log("🗺️ Ubicaciones cargadas en Dashboard (Árbitro):", data);
        setUbicaciones(data || []);
      } else {
        logger.log("❌ Error al cargar ubicaciones, status:", res.status);
      }
    } catch (error) {
  ...
  async function loadUser() {
    try {
        ...
        if (res.ok) {
          ...
          if (data?.userId) {
            ...
            if (data.role === 'arbitro' || !data.role) {
              ...
              logger.log("Usuario no es árbitro, redirigiendo...");
      ...
      if (userId) {
        logger.log("Usando datos de usuario almacenados localmente");
        setUser(prev => ({ 
          ...prev,
          userId: userId,
  ...
  async function loadGames(canchaFiltro = canchaSeleccionada) {
    ...
    try {
      ...
      logger.log("Cargando partidos desde:", url);
      ...
      const data = await res.json();
      logger.log("Partidos obtenidos:", data);
      
      // Solo establecer games
      ...
      try {
        ...
        logger.log("Intentando fallback:", fallbackUrl);
        ...
        if (fallbackRes.ok) {
          const fallbackData = await fallbackRes.json();
          logger.log("Partidos obtenidos desde fallback:", fallbackData);
          
          // Solo establecer games
          setGames(Array.isArray(fallbackData) ? fallbackData : []);
        } else {
          // Solo si ambos intentos fallan por problemas de servidor
          logger.error("Ambos intentos fallaron, manteniendo estado actual");
          // NO establecer error, mantener games como está
        }
      } catch (fallbackError) {
        logger.error("También falló el fallback:", fallbackError);
        // Solo log, no cambiar estado para evitar parpadeo
      }
    } finally {
      setLoading(false);
    }
  ...
  async function confirmApply() {
    const gameId = applyModal.gameId;
    
    // Prevenir múltiples postulaciones simultáneas
    if (applyingGames.has(gameId)) {
  ...
  async function logout() {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        logger.log("Respuesta del servidor al cerrar sesión:", data.message);
      }
    } catch (error) {
        ...
        <div className="mb-8">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between">
            ...
            <div className="mt-4 md:mt-0">
              <button 
                onClick={() => loadGames(canchaSeleccionada)}
                              ...
                              <tr key={game._id} className="hover:bg-gray-50 transition-colors duration-150">
                                ...
                                <td>
                                  <div className="flex items-center space-x-2">
                                    ...
                                    {logger.log("Cancha del partido:", game.canchaId)}
                                ...
                                <td>
                                  <div className="flex items-center space-x-2">
                                    ...
                                    {(() => {
                                      logger.log("🔍 Buscando ubicación:", game.location);
                                      logger.log("📍 Total ubicaciones:", ubicaciones.length);
                                      logger.log("📋 Ubicaciones:", ubicaciones);
                                      ...
                                      if (ubicacion && ubicacion.latitud && ubicacion.longitud) {
                                        logger.log("🗺️ Mostrando link con coords:", ubicacion.latitud, ubicacion.longitud);
                                        return (
                                          <a 
                                      ...
                                      logger.log("❌ Sin coordenadas, mostrando texto plano");
          ...
          <div className="modal-content">
            ...
            <div className="modal-footer">
              <button 
                ...
                disabled={applyingGames.has(applyModal.gameId)}
              >
                {applyingGames.has(applyModal.gameId) ? (
                  <>
