...
export default function Register(): React.ReactElement {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    Object.entries(form).forEach(([key, val]) => {
      if (key === "imagenPerfil") {
        if (val) formData.append("imagenPerfil", val as File);
      } else if (key === "edad") {
  ...
  const handleRemoveImage = (): void => {
    ...
    const inputElement = document.getElementById('imagenPerfil') as HTMLInputElement;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-400 via-blue-500 to-purple-600 py-8 px-4 relative overflow-hidden">
      ...
      {/* ToastContainer está en App.tsx */}
    </div>
  );
}
