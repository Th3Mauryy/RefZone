...
async function crearCanchaGolwin(): Promise<void> {
    try {
        ...
        if (!canchaExistente) {
            ...
            if (organizador) {
                organizador.canchaAsignada = nuevaCancha._id as any;
                await organizador.save({ validateBeforeSave: false });
            ...
            if (organizador && !organizador.canchaAsignada) {
                organizador.canchaAsignada = canchaExistente._id as any;
                await organizador.save({ validateBeforeSave: false });
...
export default crearCanchaGolwin;
